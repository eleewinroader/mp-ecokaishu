...
get_header();
	...
	if($itemRanks){
		foreach($itemRanks as $rank){
			...
			${"itemLabel$i"} = get_post_meta($post->ID, "itemsInfo03", TRUE);
			${"itemSize$i"} = get_post_meta($post->ID, "itemsInfo04", TRUE);
			$i++;
		}
	}else{
		...
		<section class="contents priceDetails" id="lpPriceDetails">
			...
			<ul class="priceEx">
				...
				<li class="twocol col">
					...
					<?php if($itemRanks): ?>
						<p>
							<span class="priceIndex block"><?php echo $itemSize0; ?></span>
					...
					<table>
						<tbody>
							<tr>
								...
								<td><?php echo taxIn($basicCharge); ?>円</td>
							</tr>
							<tr>
								<th><h5>地域料金</h5></th>
								<td><?php echo$areaEx; ?></td>
								<td><?php echo taxIn($areaCharge); ?>円</td>
							</tr>
						</tbody>
					</table>
				</div>
							...
							<?php for($i=0; $i<count($options); $i++): ?>
								<?php if(${"optionIndex$i"} == "目の前あんしんデータ消去サービス"): ?>
									<tr>
										...
										<td><?php echo ${"optionLabel$i"}[0]; ?></td>
										<td><?php echo taxIn(${"optionCharge$i"}[0]).${"optionUnit$i"}; ?></td>
									</tr>
									<tr>
										<td><?php echo ${"optionLabel$i"}[1]; ?></td>
